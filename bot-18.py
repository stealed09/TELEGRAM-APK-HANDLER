...
PROCESSING = set()  # users currently being processed
...
async def age_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.from_user.id in PROCESSING:
        await query.answer()
        return
    await query.answer()
...
async def force_join_check_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    joined = await check_force_join(user.id, context, data)
    if not joined:
        await query.answer("❌ Abhi bhi join nahi kiya! Pehle join karo.", show_alert=True)
        return
...
# NOTE: above main() is intentionally duplicated — will be replaced below
