...
async def gender_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Auto-send APK after male verification
    apk_id  = data["settings"].get("apk_file_id")
    caption = data["settings"].get("apk_caption", "Download!")
    if apk_id:
        try:
            await query.message.reply_document(document=apk_id, caption=caption)
        except Exception as e:
            logger.warning(f"Auto APK send failed for {user.id}: {e}")
